...
class University(Base):
    """Управляемый каталог вузов региона.

    Руководство добавляет записи вручную: пригодного внешнего API каталога
    нет. Вуз остаётся глобально уникальным по имени, принадлежит одному
    региону и может быть скрыт из выбора без потери связей с участниками.
    """
    ...
    # Архивный вуз остаётся у уже привязанных участников, но больше не
    # предлагается при регистрации и редактировании состава.
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
